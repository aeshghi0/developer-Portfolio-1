...
int SearchName(struct information personalData[],const int AR_SIZE)
{
	...
	if(searchName != "done")
	{
		...
		while (searchStat && index < AR_SIZE)
		{
			...
			if (searchName == personalData[index].name)
			{
		...
		//if statement to print out the not found name and lets the user know
		//that there is no matching name in the list with the searched name.
		if (searchStat == true)
		{
	...
	//returns an integer type variable and assign it to an integer variable
	//in the main.
	return index;
}
