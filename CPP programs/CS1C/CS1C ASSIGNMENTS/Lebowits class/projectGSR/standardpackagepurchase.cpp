...
#include "ui_standardpackagepurchase.h"
#include <QTextStream>
#include "customer.h"

standardPackagePurchase::standardPackagePurchase(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::standardPackagePurchase)
{
    ui->setupUi(this);

    ui->Heading->setText("Goon Squad Robots | Standard Package Purchase");
    ui->Heading->setStyleSheet("font: 30pt;");
    ui->Heading->setGeometry(400, 30, 850, 50);
   ...
   // ui->lineEdit->setText("Enter Your Company Name: ");
    ui->lineEdit->setReadOnly(true);
    ui->lineEdit->setPlaceholderText("Must Enter a Valid Quantity To Cart");



}

standardPackagePurchase::~standardPackagePurchase()
{
    delete ui;
}

void standardPackagePurchase::on_CartButton_clicked()
{
    itemsInCart = ui->CartLineEdit->text().toInt();
        ...
        if(itemsInCart > 0)
        {
           ...
           QString companyName = ui->lineEdit->text();


           runningCost = static_cast<double>(itemsInCart) * 2999.99;
...
void standardPackagePurchase::on_CardPushButton_clicked()
{
