/*******************************************************************************
 * PROGRAMMER : Ali Eshghi
 * STUDENT ID : 1112261
 * CLASS      : CS1C
 * SECTION    : MTWTH - 3pm
 * HW04		  : CS1C corporation
 * DUE DATE   : 29 January 2020
 ******************************************************************************/
#include"programmerClass.h"

/*******************************************************************************
 * Methods for class programmer
 *******************************************************************************/

//non-default constructor
programmer::programmer(int n) 
{
	n = 0;
}

//Default constructor
programmer::programmer()
{
	depNumber.clear();
	supName.clear();

	percSalaryInc = 0;

	cpp  = false;
	java = false;
}

//Destructor
programmer::~programmer() {}

//Method for getting the department number from the client and
//store it in depNumber number attribute
void programmer::setDepartmentNum(string depNum)
{
	depNumber = depNum;
}

//Method for getting the supervisore's name from the clinet and
//store it in the supName attribute
void programmer::setSupName(string supervisor)
{
	supName = supervisor;
}

//Method for getting the salary increase percentage from the client
//store it in the percSalaryInc attribute
void programmer::setPercentage(int percent)
{
	percSalaryInc = percent;
}

//Method for getting the C++ identifier true/false result from the
//client and store it in the cpp attribute
void programmer::cppIdentifier(bool cppIdent)
{
	cpp = cppIdent;


}

//Method for getting the Java identifier true/false result from the
//client and store it in the java identifier
void programmer::javaIdentifier(bool javaIdent)
{
	java = javaIdent;

}

//method for printing the attributes with the informations stored
//in them from the client to the screen
void programmer::printProgrammer()
{


	cout << left;
	cout << setw(15) << employee::getName() << setw(15) << depNumber << setw(20)
		 << supName  << "%"  << setw(15) << percSalaryInc;

	if(cpp == true)
	{
		cout << setw(22) << "Yes";
	}

	else
	{
		cout << setw(22) << "No";
	}

	if(java == true)
	{
		cout << "Yes";
	}

	else
	{
		cout << "No";
	}

	cout << endl;
}

void programmer::changePhoneNumber(string number)
{
	employee::changePhoneNumber(number);
}

bool compareNums(const employee& e, const employee& p)
{
	return e.phoneNum == p.phoneNum;
}


//This function will increment the employee's age and tells how many years has been added to the real age
int operator+(employee& emp, int inAge)
{
	int age = emp.age;
	age = age - inAge;
	cout << inAge << "years has been added to the "
	 	 << "original age of the employee" << endl << endl;

	return age;
}

//This function will increment the age by 2 years
int operator+(const int add2, employee& emp)
{
	int age = emp.age;
	age = age + add2;
	cout << "\nselected employee's age has been incresed by 2" << endl << endl;

	return age;
}


//This fucntion will compare two of employees' phone numbers
bool operator==(employee& emp1, employee& emp2)
{
	bool cmpr;

	if(emp1.phoneNum == emp2.phoneNum)
	{
		cmpr = true;
		cout << "\n\nPhone numbers are the same" << endl << endl;
	}

	else
	{
		cmpr = false;
		cout << "\n\nPhone number do not match" << endl << endl;
	}
	
	return cmpr;
}

//
istream& operator>>(istream& tempS, employee& emp)
{
	tempS >> emp.name;
	tempS >> emp.id;
	tempS >> emp.age;
	tempS >> emp.jobTitle;

	return tempS;
}

ostream& operator<<(ostream& tempS, const employee& emp)
{
	tempS << endl 			<< endl;
	tempS << emp.name 		<< endl;
	tempS << emp.id 		<< endl;
	tempS << emp.age 		<< endl;
	tempS << emp.jobTitle	<< endl;
	tempS << endl 			<< endl;

	return tempS;
}
