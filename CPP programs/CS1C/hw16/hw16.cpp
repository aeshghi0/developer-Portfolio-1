...
public:
    less_than(int vv) : v(vv){ }     //store value
    ...
    // copy elements from [first,last) to result that sastisfy predicate
{
    while(first != last)
    {
        if(pred(*first)) *result++ = *first; //copy element if predicted is satisfied
        ++first;
    }
...
class Range_low_to_high {
    ...
    Val highNum;
public:
    Range_low_to_high(Val low, Val high) : lowNum(low), highNum(high) { }
    bool operator()(Val value) const { return value >= lowNum && value <= highNum; }
};
...
// Q#1: an STL container is a collection of objects of the same type (The elements).
//      Containers own the elements. Creation and destrcution is controlled by the container.
//      The concept of an iterator is fundamental to understanding the C++ Standard Template Library(STL)
//      because iterators provide a means for accessing data stored in container classes such a vector, map,
//      list, etc. You can think of an iterator as pointing to an item that is part of a larger container items
//      stl algoruthm contains A large number of algorithms to perform activities such as searching and sorting 
//      are provided in the STL, each implemented to require a certain level of iterator
