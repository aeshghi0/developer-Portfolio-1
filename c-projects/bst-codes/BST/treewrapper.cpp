...
int main(int argc, char** argv)
{
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        if(strcmp(op,"lca") == 0) // print tree post order
        {
            Node *lca_ptr = myBST.lca(val, val2);
            output << lca_ptr->key << endl; // write printed least common ancestor to the file
            cout << "The least common Ancestor of " + to_string(val)+ " and " + to_string(val2) << endl;
            cout << lca_ptr->key << endl; // printing to consol
            continue; // move on to next command
        }
        if(strcmp(op,"mw") == 0) // print tree post order
        {
            int max_width = myBST.width();
            output << max_width << endl; // write printed least common ancestor to the file
            cout << "The maximum width level in tree " << endl;
            cout << max_width << endl; // printing to consol
            continue; // move on to next command
        }
     }
