...
//recursive function to find the least common ancestor of two value keys
Node * BST::lca(Node *root, int val1, int val2){
    
    if(root == NULL){
        return NULL;
    }
    if(root->key > val1 && root->key > val2){   //if both values are smaller than the root, checks the left subtree
        return lca(root->left, val1, val2);
    }
    if(root->key < val1 && root->key < val2){   //if both values are bigger than the root, we check the right subtree
        return lca(root->right, val1, val2);
    }

    return root;    //if one is smaller and another one is bigger then the root we know it is the least common ancestor
}

//function to get the lca of two ints, just a wrapper for a real recursion function
Node * BST::lca(int val1, int val2){
    if(find(val1) == NULL || find(val2) == NULL){   //to first check if the two value exist in the tree
        return NULL;
    }
    return lca(root, val1, val2);   //calling the recursive function
}

//UTIL function to find the height of a tree
...
//function to get the width in each level
int BST::get_width(Node *root, int level){
    if(root == NULL){
        return 0;
    }
    if(level == 1){
