...
void BinHeap::heapify(int arr[], int indx){

    int largest = indx;
    int left = 2 * indx;      //get the left child index
    int right = 2 * indx + 1; //get the right child index
...
void BinHeap::extractMax(){
    ...
    if(size <= 0){  //if the heap is empty, it just returns
        return;
    }
    if(size == 1){  //if there is only one element in the heap, we just delete that by reducing the size to 0
        size--;
        return;
    }
    A[1] = A[size]; //copying the last element to the root of the tree, extracting the maximum
    size--; //reducing the size to remove the last element (copied to root) from the heap
    heapify(A,1);   //heapify the heap in order
}
