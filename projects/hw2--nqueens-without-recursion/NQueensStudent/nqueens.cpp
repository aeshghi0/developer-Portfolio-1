...
List  place_queens(int cb_size, stack<class Pair> &chess_stack, int init_queens){
  ...
  first.col = 0; //the first square to double check after changing the initial square
  first.row = 0;

  chess_stack.push(init); //pushing the initial item into the stack

  vector<vector<int>> board(  //board double vector to check for queens placement
          cb_size,
          vector<int>(cb_size));
  ...
  while(!chess_stack.empty()){
    stack_top = chess_stack.top();  //get the top of the stack before pop
    chess_stack.pop();  //pop the stack

    Pair to_push; //to push to the stack 
...
int main(int argc, char **argv){
  ...
  while(getline(input_file,chess_set_up)){
    ...
    List output;
    if(init_queens == cb_size){ //special case, if all the queens are already in the board (chess is solved)
      ...
      output = place_queens(cb_size, chess_stack, init_queens);  //using the stack based function to place the queens
      while(!init_queens_stack.empty()){
        Pair stack_top = init_queens_stack.top();
        init_queens_stack.pop();
        if(!output.is_empty()){
